...
namespace Concurrency
{
    ...
    std::queue<int> Thread::_released_ids;
    ...
    void Thread::dispatcher() {
        while (!_ready.empty()) {
            ...
            Debug::Trace<Thread, SHOW>() << "THREAD" << nextThreadToRun->_id << "EM EXECUÇÂo" << Debug::endl;
            Debug::Trace<Thread, SHOW>() << "READY QUEUE: " << _ready.size() << Debug::endl;
    ...
    void Thread::return_to_main() {
        Debug::Trace<Thread, SHOW>() << "THREAD MAIN EM EXECUÇÃO" << Debug::endl;
    ...
    void Thread::yield() {
        ...
        Debug::Trace<Thread, SHOW>() << "Yield Chamado"; // Imprime a thread que está executando.
        ...
        if (_running != &_main && _running->_state != FINISHING && _running->_state != SUSPEND &&
            ...
            Debug::Trace<Thread, SHOW>() << "\nTHREAD " << _running->_id << " RANKEADA COM NOW TIMESTAMP = "
                            << _running->_link.rank() << Debug::endl;

            // Reinsira a thread que estava executando na fila de prontos;
            _running->_state = READY;
            _ready.insert(&_running->_link);
            Debug::Trace<Thread, SHOW>() << "\nTHREAD " << _running->_id << " REINSERIDA NA FILA DE PRONTAS." << Debug::endl;
        }
        ...
        Debug::Trace<Thread, SHOW>() << "\nTHREAD " << prev->_id << " TEVE SEU CONTEXTO TROCADO PARA " << next->_id << Debug::endl;
    ...
    int Thread::join() {
        if (_running == this) {
            Debug::Trace<Thread, SHOW>() << "Thread::join() CHAMADO PELA PRÓPRIA THREAD." << Debug::endl;
            return -1;
        }
    ...
    void Thread::sleep(Asleep_Queue *sleepQueue) {
        ...
        Debug::Trace<Thread, SHOW>() << "Thread::sleep() CHAMADO." << Debug::endl;
        _state = WAITING;
        if (_running != this) {
            _ready.remove(&_link);
        } else {
    ...
    void Thread::wakeup(bool reschedule) {
        Debug::Trace<Thread, SHOW>() << "Thread::wakeup() CHAMADO." << Debug::endl;
        _state = READY;
    ...
    void Thread::thread_exit(int exit_code) {
        Debug::Info<Thread, SHOW>() << "THREAD " << this->_id << " DELETADA." << Debug::endl;
