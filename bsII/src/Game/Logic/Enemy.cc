...
namespace BrickShooter
{
    ...
    Shot::Direction Enemy::getBestDirectionToAvoidEnemies() {
        ...
        if (distances[minIndex] < MINIMUM_DISTANCE) {
            ...
            std::vector <Shot::Direction> verticalDirs = {Shot::UP, Shot::DOWN};
            std::vector <Shot::Direction> horizontalDirs = {Shot::LEFT, Shot::RIGHT};
            if (closestEnemy->getPosition().x > currentPosition.x &&
                closestEnemy->getPosition().y > currentPosition.y) {
            ...
            } else if (closestEnemy->getPosition().x < currentPosition.x &&
                       closestEnemy->getPosition().y > currentPosition.y) {
